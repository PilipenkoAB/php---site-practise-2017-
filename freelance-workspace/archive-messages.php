...
if(isset($_SESSION['SessionId'])) {
	...
	$inboxbd=mysql_query("SELECT * FROM `Messages` WHERE `IdFrom`='$id_user' AND `Deleted`='0' AND `Archive`='1' OR `IdTo`='$id_user' AND `Deleted`='0' AND `Archive`='1'");

	// раздел удаления из архива (т.е. удаления насовсем для пользователя)
	$DeleteError = 0;
	if(isset($_POST['HiddenDelete']) && isset($_POST['IdDeleted'])){
	$DeleteArchive=htmlspecialchars(trim($_POST['HiddenDelete']));
	$IdDeleted=htmlspecialchars(trim($_POST['IdDeleted']));
	if($DeleteArchive == 1) {
	
$RegisterDelete="UPDATE `Messages` SET `Deleted`='1' WHERE `id`='$IdDeleted' ";
$resultDelete=mysql_query($RegisterDelete);
if($resultDelete==true){
	header("location: archive-messages.php");
}
else
{
...
<input type="hidden" name="HiddenDelete" id="HiddenDelete" Value= "1">
<input type="hidden" name="IdDeleted" id="IdDeleted" Value= "<?php echo $inbid; ?>">
