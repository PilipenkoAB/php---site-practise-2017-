...
if($Selection == 1){
	$Jobs_Information = $BD->query("SELECT * FROM `Jobs` WHERE `EmployerId`='$User_Id' AND `Closed`='1' ORDER BY `Title` ");				// Сортировка по Названию
}elseif ($Selection == 2){
	$Jobs_Information = $BD->query("SELECT * FROM `Jobs` WHERE `EmployerId`='$User_Id' AND `Closed`='1' ORDER BY `DataCreation` DESC");		// Сортировка по времени создания
}elseif ($Selection == 3){
	$Jobs_Information = $BD->query("SELECT * FROM `Jobs` WHERE `EmployerId`='$User_Id' AND `Closed`='1' ORDER BY `CloseData` DESC");		// Сортировка по времени создания
}else{
	$Jobs_Information = $BD->query("SELECT * FROM `Jobs` WHERE `EmployerId`='$User_Id' AND `Closed`='1'  ORDER BY `Title`");				// Сортировка по Названию
}
?>
...
<li class="FW_top_subnav_ul_current">
<a href="archive-jobs.php" class="FW_top_nav_2_href_current">Архив проектов</a>
</li>
</ul>
</nav>
</div>
...
<option <?php if($Selection == 3){ ?> selected="selected"  <?php } ?> value="3" label="По дате закрытия">По дате закрытия</option>
</select>
